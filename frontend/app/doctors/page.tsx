...
function DoctorRow({ doctor, saved, onSave }: { doctor:any; saved:boolean; onSave:(id:string)=>void }) {
  // Support both: specializations (ObjectId array) and specialization (string)
  const specs = doctor.specializations?.length
    ? doctor.specializations.slice(0,2)
    : doctor.specialization
    ? [{ name: doctor.specialization, slug: doctor.specialization.toLowerCase(), icon: '🩺' }]
    : []
    ...
    >
      ...
      <div className="flex-1 min-w-0 p-[18px_16px_18px_14px]">
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          {specs.map((s:any) => (
            <span key={s.slug} className={`${dmSans.className} text-[11px] font-medium px-2.5 py-0.5 rounded-full`} style={{ color:C.pink, background:'rgba(210,83,128,0.07)' }}>
              {s.icon} {s.name}
            </span>
          ))}
        ...
        <div className="flex flex-wrap gap-1">
          {conditions.map((c:string) => (
            <span key={c} className={`${dmSans.className} text-[11px] px-2 py-0.5 rounded-[6px] font-normal`} style={{ color:C.muD, background:C.sf }}>{c}</span>
          ))}
      ...
      <div className="p-[18px_18px_18px_0] flex flex-col items-end justify-between shrink-0 min-w-[176px]">
        <div className="flex flex-col items-end gap-2">
          <button onClick={()=>onSave(doctor._id)} className="bg-transparent border-none cursor-pointer p-1 transition-colors" style={{ color:saved?C.pink:'rgba(210,83,128,0.25)' }} aria-label="Save">
...
function DoctorsPageInner() {
  ...
  const getFilters = () => ({
    ...
    city:            'Jaipur', // hardcoded — Jaipur only
  ...
  const updateURL = useCallback((f: Record<string,string|undefined>, p=1) => {
    ...
    // Never put city in URL since it's always Jaipur
  ...
  const clearAll = () => {
    const reset = { city: 'Jaipur' }
    setFilters(reset as any); setPage(1); router.push('/doctors')
  }
  ...
  // hasFilters excludes city since it's always set
  ...
  return (
    <div className={`${cormorant.variable} ${dmSans.variable} min-h-screen pt-16`} style={{ background:C.bg, fontFamily:'var(--font-dm)' }}>
      ...
      {/* ── Top bar ── */}
      <div className="sticky top-16 z-40 border-b border-white/10" style={{ background:'#B03060', boxShadow:'0 4px 20px rgba(176,48,96,0.25)' }}>
        <div className="max-w-[1280px] mx-auto px-8 py-3 flex items-center justify-between gap-4">
          ...
          <select value={filters.sortBy||''} onChange={e=>setF('sortBy',e.target.value)}
            ...
            style={{ background:'rgba(255,255,255,0.18)', border:'1px solid rgba(255,255,255,0.28)', borderRadius:9, padding:'9px 14px', fontSize:13, color:'#fff', cursor:'pointer', outline:'none', fontFamily:'inherit' }}>
            {SORT_OPTIONS.map(o=>(
              <option key={o.value} value={o.value} style={{ color:'#2A1520', background:'#fff' }}>{o.label}</option>
            ))}
          </select>
        </div>
      </div>
        ...
        <aside className="w-[252px] shrink-0 bg-white rounded-2xl overflow-hidden sticky top-32" style={{ border:`1px solid ${C.bd}`, boxShadow:C.shadowSidebar }}>
          ...
          <div className="px-4 py-4 flex items-center justify-between" style={{ borderBottom:`1px solid ${C.sf}` }}>
            <div className="flex flex-col gap-0.5">
              ...
              {/* Jaipur location badge */}
              <span className={`${dmSans.className} inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full w-fit`}
                style={{ background:'rgba(210,83,128,0.08)', color:C.pink }}>
                <MapPin size={9}/> Jaipur, Rajasthan
              </span>
            </div>
            {hasFilters && (
              <button onClick={clearAll} className={`${dmSans.className} bg-transparent border-none cursor-pointer text-[11px] font-semibold flex items-center gap-1 shrink-0`} style={{ color:C.coral }}>
                <X size={11}/> Clear
              </button>
            )}
          </div>

          {/* Area — replaces city, shows Jaipur areas */}
          ...
          {/* Specialization — pill buttons, no dropdown */}
          <div className="px-4 py-3.5" style={{ borderBottom:`1px solid ${C.sf}` }}>
            <div className={`${dmSans.className} text-[10px] font-semibold tracking-[0.12em] uppercase mb-3`} style={{ color:C.coral }}>Specialization</div>
            <div className="flex flex-col gap-1.5">
              ...
              <button
                ...
                style={{
                  ...
                  fontSize:12, fontWeight: !filters.specialization ? 600 : 400,
                  background: !filters.specialization ? `linear-gradient(135deg,${C.pink},${C.coral})` : C.sf,
                  color: !filters.specialization ? '#fff' : C.muD,
                  border:'none', cursor:'pointer', textAlign:'left', transition:'all 0.15s',
                  boxShadow: !filters.specialization ? '0 3px 10px rgba(210,83,128,0.25)' : 'none',
                }}
              >
                <span>🩺 All Specializations</span>
              </button>

              {/* Only categories that have doctors */}
              {categories.map((c:any) => {
                const active = filters.specialization === c.slug
                return (
                  <button
                    key={c._id}
                    onClick={()=>setF('specialization', c.slug)}
                    className={dmSans.className}
                    style={{
                      display:'flex', alignItems:'center', justifyContent:'space-between',
                      width:'100%', padding:'8px 12px', borderRadius:10,
                      fontSize:12, fontWeight: active ? 600 : 400,
                      background: active ? `linear-gradient(135deg,${C.pink},${C.coral})` : C.sf,
                      color: active ? '#fff' : C.muD,
                      border: active ? 'none' : '1px solid rgba(210,83,128,0.1)',
                      cursor:'pointer', textAlign:'left', transition:'all 0.15s',
                      boxShadow: active ? '0 3px 10px rgba(210,83,128,0.25)' : 'none',
                    }}
                  >
                    ...
                    {c.doctorCount > 0 && (
                      <span style={{
                        ...
                        color: active ? 'rgba(255,255,255,0.75)' : C.mu,
                        background: active ? 'rgba(255,255,255,0.15)' : 'rgba(210,83,128,0.08)',
              ...
              <div className="flex gap-1.5 flex-wrap justify-end max-w-[380px]">
                ...
                {filters.area && (
                  <span className={`${dmSans.className} text-[11px] px-2.5 py-1 rounded-full flex items-center gap-1.5`} style={{ background:'rgba(224,142,109,0.1)', color:C.coral }}>
                    ...
                    <button onClick={()=>setF('area','')} className="bg-transparent border-none cursor-pointer text-[13px] leading-none p-0" style={{ color:C.coral }}>×</button>
                  </span>
                )}
                {filters.availableOnline==='true' && (
                  <span className={`${dmSans.className} text-[11px] px-2.5 py-1 rounded-full flex items-center gap-1.5`} style={{ background:'rgba(143,175,69,0.1)', color:C.sgD }}>
                    Online
                ...
                {filters.availableToday==='true' && (
                  <span className={`${dmSans.className} text-[11px] px-2.5 py-1 rounded-full flex items-center gap-1.5`} style={{ background:'rgba(143,175,69,0.1)', color:C.sgD }}>
                    Available Today
                    <button onClick={()=>setF('availableToday','')} className="bg-transparent border-none cursor-pointer text-[13px] leading-none p-0" style={{ color:C.sgD }}>×</button>
                  </span>
                )}
              </div>
            )}
          ...
          {loading
            ...
            ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className={`${dmSans.className} text-sm font-light mb-6`} style={{ color:C.mu }}>Try adjusting your filters</p>
